...
from rectified.sr_binary_pair_dataset import SRBinaryPairDataset
...
def parse_args():
    ...
    p.add_argument("--name", type=str, default=None, help="Experiment name under checkpoints_dir")
    p.add_argument("--checkpoints_dir", type=str, default="./checkpoints")
    p.add_argument("--save_path", type=str, default=None, help="Direct checkpoint path (overrides --name).")
...
def resolve_save_path(args):
    ...
    if args.name:
        return os.path.join(args.checkpoints_dir, args.name, "model_epoch_best.pth")
    raise ValueError("Either --save_path or --name must be provided.")
...
def main():
    ...
    print(f"[Classifier] save_path={save_path}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(ckpt, save_path.replace(".pth", "_last.pth"))
        if val_ap > best_ap:
            best_ap = val_ap
            torch.save(ckpt, save_path)
            print(f"[Classifier] saved best -> {save_path}")
